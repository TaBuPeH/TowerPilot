...
- `guild_progress`: inspect the Guild badge every five minutes; collect only visibly claimable weekly progress boxes. Locked boxes are told apart by the padlock image, which the flow learns from the track itself while at least two boxes are locked (`chest_lock_captured`); until then the claim pass is skipped (`guild_claims_skipped`) and the flow returns to the game. A flow that raises is dropped with `mission_crash` and the return strip is taken.
